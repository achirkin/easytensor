...
instance Ord VFloatX2 where
  ...
  min (VFloatX2# a1 a2) (VFloatX2# b1 b2) =
      VFloatX2# (if isTrue# (a1 `gtFloat#` b1) then b1 else a1)
                (if isTrue# (a2 `gtFloat#` b2) then b2 else a2)
  {-# INLINE min #-}
  -- | Element-wise maximum
  max (VFloatX2# a1 a2) (VFloatX2# b1 b2) =
      VFloatX2# (if isTrue# (a1 `gtFloat#` b1) then a1 else b1)
...
instance Num VFloatX2 where
  VFloatX2# a1 a2 + VFloatX2# b1 b2
    = VFloatX2# (plusFloat# a1 b1) (plusFloat# a2 b2)
  {-# INLINE (+) #-}
  VFloatX2# a1 a2 - VFloatX2# b1 b2
    = VFloatX2# (minusFloat# a1 b1) (minusFloat# a2 b2)
  {-# INLINE (-) #-}
  VFloatX2# a1 a2 * VFloatX2# b1 b2
    = VFloatX2# (timesFloat# a1 b1) (timesFloat# a2 b2)
  {-# INLINE (*) #-}
  negate (VFloatX2# a1 a2)
    = VFloatX2# (negateFloat# a1) (negateFloat# a2)
  {-# INLINE negate #-}
  abs (VFloatX2# a1 a2)
    = VFloatX2# (if isTrue# (a1 `geFloat#` 0.0#) then a1 else negateFloat# a1)
                (if isTrue# (a2 `geFloat#` 0.0#) then a2 else negateFloat# a2)
  {-# INLINE abs #-}
  signum (VFloatX2# a1 a2)
    = VFloatX2# (if isTrue# (a1 `gtFloat#` 0.0#)
                 then 1.0#
...
instance VectorCalculus Float 2 VFloatX2 where
  ...
  {-# INLINE normL2 #-}
  normLPInf (VFloatX2# a1 a2)
    = F# (if isTrue# (a1 `gtFloat#` a2) then a1 else a2)
  {-# INLINE normLPInf #-}
  normLNInf (VFloatX2# a1 a2)
...
instance Vector2D Float where
  ...
  {-# INLINE vec2 #-}
  det2 (VFloatX2# a1 a2)  (VFloatX2# b1 b2)
...
instance ElementWise (Idx '[2]) Float VFloatX2 where

  (!) (VFloatX2# a1 _) ( 1 :! Z) = F# a1
  (!) (VFloatX2# _ a2) ( 2 :! Z) = F# a2
  (!) _               ( _ :! Z) = undefined
  {-# INLINE (!) #-}

  broadcast (F# x) = VFloatX2# x x
  {-# INLINE broadcast #-}

  ewmap f (VFloatX2# x y) = case (f (1:!Z) (F# x), f (2:!Z) (F# y)) of
                              (F# r1, F# r2) -> VFloatX2# r1 r2
  {-# INLINE ewmap #-}

  ewgen f = case (f (1:!Z), f (2:!Z)) of (F# r1, F# r2) -> VFloatX2# r1 r2
  {-# INLINE ewgen #-}

  ewfold f x0 (VFloatX2# x y) = f (2:!Z) (F# y) (f (1:!Z) (F# x) x0)
  {-# INLINE ewfold #-}

  elementWise f (VFloatX2# x y) = (\(F# a) (F# b) -> VFloatX2# a b)
                               <$> f (F# x) <*> f (F# y)
  {-# INLINE elementWise #-}

  indexWise f (VFloatX2# x y) = (\(F# a) (F# b) -> VFloatX2# a b)
                             <$> f (1:!Z) (F# x) <*> f (2:!Z) (F# y)
  {-# INLINE indexWise #-}
