...
instance Show QFloat where
    showsPrec p (unpackQ# -> (# x, y, z, w #))
      ...
      where
        ...
        go :: ShowState -> [(Float, Maybe Char)] -> (ShowState, ShowS)
        go s ((v,l):xs)
        ...
        showComponent :: ShowState -> Float -> Maybe Char -> (ShowState, ShowS)
...
instance Read QFloat where
    readPrec     = parens $ readPrec >>= go id 0 0 0 0
      where
        go :: (Float -> Float)
           -> Float -> Float -> Float -> Float
           -> Float -> ReadPrec QFloat
        go f x y z w new =
        ...
        proceed :: Float -> Float -> Float -> Float
                -> Lexeme -> ReadPrec QFloat
