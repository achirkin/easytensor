...
data DataFrame t (ds :: [k]) = DataFrame
  { _dShape :: !(Dim ds)
  , dData   :: !(Array t ds)
  }
...
instance PreservingDim (DataFrame t) (DataFrame t) where
  shape (DataFrame ds _) = ds
  looseDims (DataFrame ds arr) = DataFrame (wrapDims ds) (unsafeCoerce arr)
    where
      wrapDims :: Dim ns -> Dim (WrapNats ns)
      wrapDims D         = D
      wrapDims (n :* ns) = n :* wrapDims ns
  withShape (DataFrame xds arr) f =  case someDimVal xds of
    Just (SomeDim ds) -> f (DataFrame ds (unsafeCoerce arr))
    Nothing -> error "Could not extract runtime naturals to construct dimensions."
